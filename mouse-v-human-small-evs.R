...
sxcd <- read.csv('E:/paper-files/mevs_3m_KO_v_3m_WT_mirna_deseq_xlfcshrink.csv', sep=',', header = TRUE)
adeno <- read.csv('E:/paper-files/mevs_1y_KO_v_1y_WT_mirna_deseq_xlfcshrink.csv', sep=',', header = TRUE)
sbt <- read.csv('E:/paper-files/hevs_SBT_v_control_mirna_deseq_xlfcshrink.csv', sep=',', header = TRUE)
hgsoc <- read.csv('E:/paper-files/hevs_HGSOC_v_control_mirna_deseq_xlfcshrink.csv', sep=',', header = TRUE)

#i've already set the miRNAs to remove species prefix so mmu- and -hsa so that you can directly compare
#graph the number od DEGs all regardless of log2fold change
sxcd_up <- filter(sxcd, log2FoldChange > 0)
sxcd_down <- filter(sxcd, log2FoldChange < 0)
adeno_up <- filter(adeno, log2FoldChange > 0)
adeno_down <- filter(adeno, log2FoldChange < 0)
sbt_up <- filter(sbt, log2FoldChange > 0)
sbt_down <- filter(sbt, log2FoldChange < 0)
hgsoc_up <- filter(hgsoc, log2FoldChange > 0)
hgsoc_down <- filter(hgsoc, log2FoldChange < 0)

degs_graph <- read.csv('E:/paper-files/ev-degs.csv', sep=',', header = TRUE)

degs_graph$Regulation <- factor(degs_graph$Regulation, levels = c("Upregulated", "Downregulated"))
degs_graph$Group <- factor(degs_graph$Group, levels = c("Sex cords", "Adenoma", "SBT", "HGSOC"))

degs_gg <- ggplot(degs_graph, aes(x = Group, y = Count, fill = Regulation)) +
  ...
  theme(
    ...
    axis.title.y = element_text(size = 24),    # Increase y-axis title font size
    axis.text = element_text(size = 24),       # Increase axis text (ticks) font size
...
ggsave("E:/paper-files/images/mirna_degs_count.png", plot = degs_gg, 
       width = 8, height = 8, units = "in", dpi = 800)



#logcpm + c expression of each miRNA
ev_logcpm <- read.csv('E:/paper-files/logcpm-sum-evs.csv', sep=',', header = TRUE)

#ev_logcpm$group <- factor(ev_logcpm$group, levels = c("3-month ", "Downregulated"))
ev_logcpm$condition <- factor(ev_logcpm$condition, levels = c("Adenoma", "Sex cord", "Control (mouse)", 
                                                           "HGSOC", "SBT", "Control (human)"))
ev_logcpm$species <- factor(ev_logcpm$species, levels = c("mouse", "human"))
...
logcpm_mouse <- filter(ev_logcpm, species == "mouse")
logcpm_human <- filter(ev_logcpm, species == "human")

logcpm_gg <- ggplot(ev_logcpm, aes(x = condition, y = logcpm, colour = condition)) +
  geom_boxplot() + 
  geom_jitter() + 
  #facet_wrap(~ mirna, scales = "free_y", nrow = 1) + 
  facet_grid(rows = vars(species), cols = vars(mirna), scales = "free_x") + 
  theme_bw() +
  labs(title = "",
       x = "",
       y = "LogCPM + c", 
       legend = "Condition") + 
   scale_colour_manual(values = condition_colors) +  # Apply custom colors
    theme(
    ...
    axis.text = element_text(size = 22),       # Increase axis text (ticks) font size
    legend.title = element_text(size = 22),    # Increase legend title font size
    legend.text = element_text(size = 22), 
    axis.text.x = element_blank(),             # Remove x-axis text labels in facets
    axis.ticks.x = element_blank(), 
    strip.text = element_text(size = 22),      # Remove x-axis ticks in facets# Increase legend text font size
    legend.position = "right"
  )
print(logcpm_gg)

ggsave("E:/paper-files/images/mirna_logcpm_biomark.png", plot = logcpm_gg, 
       width = 18, height = 5, units = "in", dpi = 800)

logcpm_gg_mouse <- ggplot(logcpm_mouse, aes(x = condition, y = logcpm, colour = condition)) +
  geom_boxplot() + 
  geom_jitter() + 
  facet_wrap(~ mirna, scales = "free_y", nrow = 1) + 
  theme_bw() +
  ...
  theme(
    ...
    axis.text = element_text(size = 20),       # Increase axis text (ticks) font size
    legend.title = element_text(size = 20),    # Increase legend title font size
    legend.text = element_text(size = 20), 
    axis.text.x = element_blank(),             # Remove x-axis text labels in facets
    axis.ticks.x = element_blank(), 
    strip.text = element_text(size = 18),      # Remove x-axis ticks in facets# Increase legend text font size
    legend.position = "right"
  )
print(logcpm_gg_mouse)

logcpm_gg_human <- ggplot(logcpm_human, aes(x = condition, y = logcpm, colour = condition)) +
  geom_boxplot() + 
  geom_jitter() + 
  facet_wrap(~ mirna, scales = "free_y", nrow = 1) + 
  theme_bw() +
...
print(logcpm_gg_human)

#install.packages("patchwork")  # Install patchwork
library(patchwork)             # Load the patchwork package

ev_biomark <- logcpm_gg_mouse / logcpm_gg_human
print(ev_biomark)
ggsave("E:/paper-files/images/mirna_logcpm_biomark_sep.png", plot = ev_biomark, 
       width = 14, height = 7, units = "in", dpi = 800)

#####LCM small RNA for supplemental fig
lcm_logcpm <- read.csv('E:/paper-files/logcpm-sum-lcm-biomark.csv', sep=',', header = TRUE)

lcm_logcpm$condition <- factor(lcm_logcpm$condition, levels = c("Adenoma", "Sex cord", "Control (mouse)", 
                                                              "HGSOC", "SBT", "Control (human)"))
lcm_logcpm$species <- factor(lcm_logcpm$species, levels = c("mouse", "human"))
...
logcpm_mouse_lcm <- filter(lcm_logcpm, species == "mouse")
logcpm_human_lcm <- filter(lcm_logcpm, species == "human")

logcpm_gg_mouse_lcm <- ggplot(logcpm_mouse_lcm, aes(x = condition, y = logcpm, colour = condition)) +
  geom_boxplot() + 
  geom_jitter() + 
  facet_wrap(~ mirna, scales = "free_y", nrow = 1) + 
  theme_bw() +
...
print(logcpm_gg_mouse_lcm)

logcpm_gg_human_lcm <- ggplot(logcpm_human_lcm, aes(x = condition, y = logcpm, colour = condition)) +
  geom_boxplot() + 
  geom_jitter() + 
  facet_wrap(~ mirna, scales = "free_y", nrow = 1) + 
  theme_bw() +
...
print(logcpm_gg_human_lcm)

#install.packages("patchwork")  # Install patchwork
library(patchwork)             # Load the patchwork package

lcm_biomark <- logcpm_gg_mouse_lcm / logcpm_gg_human_lcm
print(lcm_biomark)


ggsave("E:/paper-files/images/mirna_logcpm_biomark_sep_lcm.png", plot = lcm_biomark, 
       width = 14, height = 4, units = "in", dpi = 800)

##########qpcr data tissue
#logcpm + c expression of each miRNA
ev_qpcr <- read.csv('E:/paper-files/tissue-mirna-qpcr.csv', sep=',', header = TRUE)

#ev_logcpm$group <- factor(ev_logcpm$group, levels = c("3-month ", "Downregulated"))
ev_qpcr$condition <- factor(ev_qpcr$condition , levels = c("HGSOC", "SBT", "Control"))
ev_qpcr$group <- factor(ev_qpcr$group, levels = c("Normal", "Benign", "SBT", "HGSOC"))
ev_qpcr$mirna <- factor(ev_qpcr$mirna, levels = c("miR-200a-3p", "miR-200b-3p", "miR-200c-3p", 
                                                  "miR-375-3p", "miR-429"))
...
ev_qpcr$log_expression <- log2(ev_qpcr$expression + 4)

write.csv(ev_qpcr, "E:/paper-files/ev_qpcr.csv", row.names = FALSE)

qpcr_gg <- ggplot(ev_qpcr, aes(x = condition, y = log_expression, colour = condition)) +
  geom_boxplot() + 
  geom_jitter() + 
  facet_wrap(~ mirna, scales = "free_y", nrow = 1) + 
  #facet_grid(rows = vars(species), cols = vars(mirna), scales = "free_x") + 
...
ggsave("E:/paper-files/images/mirna_qpcr_biomark.png", plot = qpcr_gg, 
       width = 14, height = 4, units = "in", dpi = 800)
...
qpcr_gg_stats <- qpcr_gg + stat_compare_means(method = "anova", label.y = 40) 


qpcr_gg_stats <- stat_compare_means(method = "anova", label.y = 40) 


ggsave("E:/paper-files/images/mirna_qpcr_biomark_stats.png", plot = qpcr_gg_stats, 
       width = 14, height = 4, units = "in", dpi = 800)

#sxcd_up <- filter(sxcd, log2FoldChange >= 1.5)
#sxcd_down <- filter(sxcd, log2FoldChange <= -1.5)
#adeno_up <- filter(adeno, log2FoldChange >= 1.5)
#adeno_down <- filter(adeno, log2FoldChange <= -1.5)
#sbt_up <- filter(sbt, log2FoldChange >= 1.5)
#sbt_down <- filter(sbt, log2FoldChange <= -1.5)
#hgsoc_up <- filter(hgsoc, log2FoldChange >= 1.5)
#hgsoc_down <- filter(hgsoc, log2FoldChange <= -1.5)
...
#coerce those mirna lists to vector for venn analysis
sxcd_up <- sxcd_up$mirna
sxcd_down <- sxcd_down$mirna
adeno_up <- adeno_up$mirna
adeno_down <- adeno_down$mirna
sbt_up <- sbt_up$mirna
sbt_down <- sbt_down$mirna
hgsoc_up <- hgsoc_up$mirna
hgsoc_down <- hgsoc_down$mirna
##############################VENN##############################################
...
venn_up_mh <- venn.diagram(x = venn_data_ordered_up,
                             ...
                             filename = "E:/paper-files/images/venn_up_mh_evs.tiff",
...
venn_down_mh <- venn.diagram(x = venn_data_ordered_down,
                           category.names = c("Sexcords", "SBT", "Adenoma", "HGSOC"), 
                           filename = "E:/paper-files/images/venn_down_mh_evs.tiff",
                           disable.logging = FALSE, 
                           imagetype = "tiff", 
                           main = "Mouse versus Human", 
                           main.fontface = "bold", 
...
CairoPNG("E:/paper-files/images/upset_genes_mh_up_evs.png", width = 8, height = 8, units = "in", res = 800, 
         bg = "transparent")
...
CairoPNG("E:/paper-files/images/upset_genes_mh_down_evs.png", width = 8, height = 8, units = "in", res = 800)
...
write.csv(elements_up_df, file = "E:/paper-files/small_evs_venn_elements_up_df.csv", 
          row.names = FALSE)
...
write.csv(elements_down_df, file = "E:/paper-files/small_evs_venn_elements_down_df.csv", row.names = FALSE)
